document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderMessage(msg) {
    ...
    if (msg.type === "document") {
      ...
      const lenKb = msg.document_length
        ? ` • ${(msg.document_length / 1024).toFixed(1)} KB`
        : "";
      card.innerHTML = `
  ...
  async function fetchMessages() {
    try {
      const response = await fetch(
        `/api/messages?phone=${encodeURIComponent(currentPhone)}`,
      );
  ...
  async function fetchStatus() {
    try {
      ...
      if (data.phone && !phoneInitialized) {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      renderMessage({
        ...
        sender: "bot",
        type: "text",
        text: "Could not send message to SokoFlow backend. Please ensure backend services are running.",
        timestamp: new Date().toISOString(),
      });
      setTyping(false);
    }
